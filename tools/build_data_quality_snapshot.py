...
def load_minute_stats(cache_dir: Path) -> dict[str, dict[str, Any]]:
    ...
    for path in sorted(cache_dir.glob("*.json")):
        ...
        for bar in bars:
            ...
            if timestamp > latest:
                latest = timestamp
        stats[code] = {"bar_count": len(bars), "latest_timestamp": latest, "cache_path": str(path)}
    return stats
...
def classify_quote(item: dict[str, Any] | None, max_lag_seconds: float) -> dict[str, Any]:
    ...
    if lag is None:
        return {"status": "missing", "lag_seconds": None, "message": "行情延迟字段缺失。"}
    if lag > max_lag_seconds:
        return {"status": "stale", "lag_seconds": round(lag, 3), "message": f"行情延迟 {lag:.1f} 秒，超过 {max_lag_seconds:.1f} 秒阈值。"}
    return {"status": "usable", "lag_seconds": round(lag, 3), "message": "行情延迟在阈值内。"}


def classify_daily(stats: dict[str, Any] | None, as_of: datetime, min_bars: int, max_age_days: int) -> dict[str, Any]:
    if not stats:
        return {"status": "missing", "row_count": 0, "latest_trade_date": None, "age_days": None, "message": "缺少日线数据。"}
    row_count = int(stats.get("row_count") or 0)
    latest = stats.get("latest_trade_date")
    days = age_days(as_of, latest)
    if row_count < min_bars:
        return {"status": "insufficient", "row_count": row_count, "latest_trade_date": latest, "age_days": days, "message": f"日线数量 {row_count} 少于 {min_bars}。"}
    if days is None:
        return {"status": "missing", "row_count": row_count, "latest_trade_date": latest, "age_days": None, "message": "日线最新日期无法解析。"}
    if days > max_age_days:
        return {"status": "stale", "row_count": row_count, "latest_trade_date": latest, "age_days": days, "message": f"日线距当前 {days} 天，超过 {max_age_days} 天阈值。"}
    return {"status": "usable", "row_count": row_count, "latest_trade_date": latest, "age_days": days, "message": "日线样本和新鲜度可用。"}


def classify_minute(stats: dict[str, Any] | None, as_of: datetime, min_bars: int, max_age_hours: float) -> dict[str, Any]:
    if not stats:
        return {"status": "missing", "bar_count": 0, "latest_timestamp": None, "age_hours": None, "message": "缺少分钟线缓存。"}
    bar_count = int(stats.get("bar_count") or 0)
    latest = stats.get("latest_timestamp")
    hours = age_hours(as_of, latest)
    if bar_count < min_bars:
        return {"status": "insufficient", "bar_count": bar_count, "latest_timestamp": latest, "age_hours": None if hours is None else round(hours, 3), "message": f"分钟线数量 {bar_count} 少于 {min_bars}。"}
    if hours is None:
        return {"status": "missing", "bar_count": bar_count, "latest_timestamp": latest, "age_hours": None, "message": "分钟线最新时间无法解析。"}
    if hours > max_age_hours:
        return {"status": "stale", "bar_count": bar_count, "latest_timestamp": latest, "age_hours": round(hours, 3), "message": f"分钟线距当前 {hours:.1f} 小时，超过 {max_age_hours:.1f} 小时阈值。"}
    return {"status": "usable", "bar_count": bar_count, "latest_timestamp": latest, "age_hours": round(hours, 3), "message": "分钟线样本和新鲜度可用。"}
