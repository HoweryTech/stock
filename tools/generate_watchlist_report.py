...
def format_unified_candidate(candidate: dict[str, str], index: int) -> list[str]:
    ...
    lines = [
        ...
        f"- 技术健康分：{candidate.get('technical_health_score') or '-'}",
        f"- 技术健康状态：{candidate.get('technical_health_status') or '-'}",
        f"- 技术健康证据：{candidate.get('technical_health_evidence') or '-'}",
        f"- 技术风险标记：{candidate.get('technical_risk_flags') or '-'}",
